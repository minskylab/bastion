...
func (tq *TaskQuery) QueryAssignees() *MemberQuery {
	query := &MemberQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) WithAssignees(opts ...func(*MemberQuery)) *TaskQuery {
	query := &MemberQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TaskQuery) sqlAll(ctx context.Context) ([]*Task, error) {
	...
	if query := tq.withProject; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(task.ProjectPrimaryKey[1], fks...))
			},
	...
	if query := tq.withAssignees; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(task.AssigneesPrimaryKey[0], fks...))
			},
			ScanValues: func() [2]interface{} {
				return [2]interface{}{new(uuid.UUID), new(uuid.UUID)}
			},
		...
		if err := sqlgraph.QueryEdges(ctx, tq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "assignees": %w`, err)
		}
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "assignees" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Assignees = append(nodes[i].Edges.Assignees, n)
			}
		}
	}
